...
prawn_document :page_size => @invoice.invoice_template.paper_size do |pdf|
  ...
  pdf.bounding_box [1.cm, 24.cm], :width => 6.cm do
    ...
    pdf.move_down 20
    pdf.text "#{@invoice.customer.zip} #{@invoice.customer.city}", :style => :bold
  end

  pdf.bounding_box [13.cm, 19.cm], :width => 5.cm do
    pdf.text "#{@invoice.invoice_template.city}, den #{format_date(@invoice.date)}"
  end

  pdf.bounding_box [0, 18.cm], :width => pdf.bounds.width do
    pdf.text "Rechnung Nr. #{@invoice.number}", :style => :bold
    pdf.move_down 20

    data = Array.new
    data << ["Anzahl", "Einheit", "Beschreibung", "Einzelpreis", "Gesamtpreis"]
    @invoice.line_items.each do |item|
      data << [number_with_delimiter(item.quantity, :separator => ","), item.unit, item.description, format_money(item.unit_price), format_money(item.total)]
    end
    data << ["", "", "", "Summe", format_money(@invoice.total)]
    @invoice.taxes.each do |vat, sum|
      data << ["", "", "", "USt. #{vat}%", format_money(sum)]
    end
    data << ["", "", "", "Endbetrag", format_money(@invoice.total_with_tax)] 
    ...
    pdf.text "Bitte überweisen Sie den Betrag bis zum #{format_date(@invoice.date + 14)} auf das unten angegebene Konto."
  end
